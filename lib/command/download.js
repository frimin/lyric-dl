            ...
            switch (format) {
                case 'lrc':
                    if (result['lrc'] && (printAll || options['outputOriginal'] ))
                        writeTasks.push(createWriteTask(options, songid, result['name'], tryExtractLyricToStr(result['lrc']), format))
                    if (result['tlrc'] && (printAll || options['outputTranslate']))
                        writeTasks.push(createWriteTask(options, songid, result['name'], tryExtractLyricToStr(result['tlrc']), 'tr.' + format))
                    break
                case 'json':
                    writeTasks.push(createWriteTask(options, songid, result['name'], JSON.stringify(result), format))
                    break 
                default:
