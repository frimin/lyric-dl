...
function createDownloadTask(opt, url, downloaded) {
    return function (cb) {
        ...
        rt.loader.download({ id: rt.id }, function (result) {
            ...
            switch (outformat)
            {
                case 'lrc':
                    ...
                    if (opt['extract']) {
                        console.error('output mode "lrc" not support option "--extract"')
                        process.exit(2)
                    }
                ...
                case 'json':
                    ...
                    if (opt['extract']) {
                        var extract = rt.loader.extract(result)

                        if (extract.lrc) {
                            result.lrc = extract.lrc
                        }

                        if (extract.tlrc) {
                            result.tlrc = extract.tlrc
                        }
                    }
