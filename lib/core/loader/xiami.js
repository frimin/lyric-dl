...
exports.download = function (options, response) {
    ...
    async.waterfall([
        ...
        function (songid, callback) {
            request.http_request({
                host: 'www.xiami.com',
                path: 'http://www.xiami.com/song/playlist/id/' + songid + '/object_name/default/object_id/0/cat/json',
                method: 'GET',
                headers: {
                    'User-Agent': 'Mozilla/5.0 (Macintosh; Intel Mac OS X 10_12_2) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/55.0.2883.95 Safari/537.36,'
                },
...
exports.search = function (options, response) {
    ...
    var length = 20
    var page = Math.floor((options.index || 0) / length) + 1
    ...
    request.http_request({
        ...
        response(common.makeSearchResponseData(results, parseInt(totalCount)));
    })
}
